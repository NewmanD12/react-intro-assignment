...
class App extends Component {
  ...
  filterMovie = (search) => {
    const { movies } = this.state
    const searchCategory = search.searchCategory
    ...
    if(searchCategory === 'director'){
      ...
      const searchCriteria = search.searchCriteria.toLowerCase()
      const results = movies.filter((movie) => movie.actors.toString().toLowerCase().includes(searchCriteria))
      this.setState({
        searchResults : results
      })
    }
  ...
  render() {
    return (
      ...
      <Table 
        ...
        searchMovies = {this.state.searchResults}
